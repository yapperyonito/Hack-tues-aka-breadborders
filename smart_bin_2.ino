...
void setup() //runs only one time
{
  ...
  //pinMode defines if the mode of the pin is input or output
  //photoelectric sensor
  //pinMode(EMITTER, OUTPUT); //emitter displays information
  //pinMode(PHOTO_RESISTOR, INPUT); //photo resistor is set to get information
...
void loop() //runs until no electricity goes through the electric chain
{
  ...
  //delay() //prints the value of the distance sensor (ln --> puts new line every time)
  ...
  else
  {
   ...
   // digitalWrite(EMITTER, HIGH);
    //Serial.println(sensorValue); //printing sensor value in serial monitor (ln --> new line every time)
    //delay(10);
    //digitalWrite(EMITTER, LOW); //turn off emitter - no electricity goes through the emitter pin
    ...
    if (sensorValue>250)
    {
      //Serial.println("Hi"); //printing sensor value in serial monitor (ln --> new line every time)
  ...
  // }
    //opravi posle
                //rotates the servo 90 degrees in the opposite direction of the usual one
    //if (distanceValue>=100);

    //myServo.write(90); 
    //rotate servo motor --> open the top part of the bin
  //  if (time_open_start >= 7500)
  //     myServo.write(90); //rotate servo motor --> open the top part of the bin
    //myServo.write(90);
  //}
    // else
    ...
    // }
  //}
  //photo resistor/ photoelectric sensor
//counter goes up --> it will be later displayed on the matrix

//displaying numbers on the matrix
  //counter++;
